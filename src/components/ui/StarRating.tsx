...
function Star({ starIndex, hoveredStarIndex, onClick, onHover }: StarProps) {
  const isStarFill = starIndex <= hoveredStarIndex ? true : false;
  ...
  return (
    <span
      ...
      role="button"
    >
      {isStarFill && <FullStarSvg />}

      {!isStarFill && <EmptyStarSvg />}
    </span>
  );
}
