import { useState } from 'react';

interface StarRatingProps {
  onRate: (rating: number) => void;
  maxRating?: number;
  defaultRating?: number;
  color?: string;
  size?: number;
  messages?: string[];
  showRating?: boolean;
}

function StarRating({
  onRate,
  maxRating = 5,
  defaultRating = 0,
  color = '#fcc419',
  size = 48,
  messages = [],
  showRating = true,
}: StarRatingProps) {
  const textStyle = {
    lineHeight: '1',
    margin: 0,
    color,
    fontSize: `${size / 1.5}px`,
  };

  const [rating, setRating] = useState(defaultRating);
  const [hoverRating, setHoverRating] = useState(0);

  function handleOnStarClick(starIndex: number) {
    if (rating === starIndex) {
      setRating(0);
      return onRate(0);
    }
    setRating(starIndex);
    onRate(starIndex);
  }

  function handleOnStarHover(starIndex: number) {
    if (rating && starIndex === 0) return;
    setHoverRating(starIndex);
  }

  return (
    <>
      <div style={containerStyle}>
        <div style={starContainerStyle}>
          {Array.from({ length: maxRating }, (_, i) => (
            <Star
              key={i}
              isFull={i + 1 <= (hoverRating || rating)}
              color={color}
              size={size}
              onClick={() => handleOnStarClick(i + 1)}
              onHover={() => handleOnStarHover(i + 1)}
              onHoverLeave={() => handleOnStarHover(0)}
            />
          ))}
        </div>
        {messages.length === maxRating && (
          <p style={textStyle}>{messages[hoverRating - 1] || ''}</p>
        )}
        {showRating && !messages.length && (
          <p style={textStyle}>{hoverRating || ''}</p>
        )}
      </div>
    </>
  );
}

interface StarProps {
  isFull: boolean;
  color: string;
  size: number;
  onClick: () => void;
  onHover: () => void;
  onHoverLeave: () => void;
}

function Star({
  isFull,
  color,
  size,
  onClick,
  onHover,
  onHoverLeave,
}: StarProps) {
  const starStyle = {
    width: `${size}px`,
    height: `${size}px`,
    display: 'block',
    cursor: 'pointer',
  };

  return (
    <span
      onMouseEnter={onHover}
      onMouseLeave={onHoverLeave}
      onClick={onClick}
      style={starStyle}
      role="button"
    >
      {isFull && <FullStarSvg color={color} />}

      {!isFull && <EmptyStarSvg color={color} />}
    </span>
  );
}

interface StarSvgProps {
  color: string;
}

function FullStarSvg({ color }: StarSvgProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 20 20"
      fill={color}
      stroke={color}
    >
      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
    </svg>
  );
}

function EmptyStarSvg({ color }: StarSvgProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      viewBox="0 0 24 24"
      stroke={color}
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="{2}"
        d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"
      />
    </svg>
  );
}

const containerStyle = {
  display: 'flex',
  alignItems: 'center',
  gap: '16px',
};

const starContainerStyle = {
  display: 'flex',
};

export default StarRating;
